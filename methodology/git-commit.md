...
#### 10. Task-артефакты и run-репорты (по умолчанию исключать)
**Логика:** Операционные артефакты выполнения пайплайна не являются продуктовым кодом и засоряют коммиты.

**По умолчанию НЕ коммитить:**
- `docs/tasks/**` (включая `epics/*/stories/*/task-*`, `run-reports/`, `gateway-build-windows/`, `test-coverage-*`, `ACTIVE_TASK_PATH-*`)
- любые файлы с `BULLRUN-PHASE-LOG.md`, `acceptance-verification-*`, `implementation-plan-*`, `run-summary-*`

**Исключение:** коммитить task-артефакты только при явной команде оператора в текущем запросе.
...
- ❌ **НЕ коммитить:** Новые `docs/tasks/**` артефакты без явного разрешения оператора
...
- [ ] Task-артефакты (`docs/tasks/**`, `BULLRUN-PHASE-LOG`, `acceptance-verification`, `run-summary`) исключены, если не было явной команды их коммитить
