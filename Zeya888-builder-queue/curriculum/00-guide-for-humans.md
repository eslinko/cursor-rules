...
**Как прогнать пачку стори/багов через оркестратор?**  
Простой мануал: [`../guides/operator-how-to-run-queue.md`](../guides/operator-how-to-run-queue.md) — один процесс для story и bug; очередь задаёте свободно (`remaining`, список, диапазон).
